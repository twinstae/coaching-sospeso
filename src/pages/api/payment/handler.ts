...
export function createHandler({ sospesoRepo, paymentRepo }: {
  sospesoRepo: SospesoRepositoryI, paymentRepo: PaymentRepositoryI
}) {
  ...
  return (async ({ request }: { request: Request }) => {
    ...
    if (PCD_PAY_RST === "success" && PCD_PAY_CODE === "0000" && PCD_PAY_TYPE === "card") {
      ...
      const event = await services.completeSospesoPayment({ paymentId: paymentId, paymentResult: params });
