...
export function createSospesoServices({sospesoRepo, paymentRepo }: {
  sospesoRepo: SospesoRepositoryI,
  paymentRepo: PaymentRepositoryI
}) {
  return {
    completeSospesoPayment: async (input: { paymentId: string, paymentResult: Record<string, string> }) => {
    ...
    cancelSospesoPayment: async ({ sospesoId: paymentId }: { sospesoId: string}) => {
