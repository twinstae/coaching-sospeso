...
export function parseRouteParamsFromUrl<RouteKey extends RouteKeys>(routeKey: RouteKey, url: URL): RouteParams<RouteKey>{
    
  const route = resolveRoute(routeKey);

  const searchParams = Object.fromEntries(new URLSearchParams(url.search));
  
  console.log(url.search, "hey")
  if("paramsSchema" in route){
    ...
    const pathValues = new RegExp(route.path.replaceAll(/\[[^/]+\]/g, "([^/]+)")).exec(url.pathname)?.slice(1);
    

    if (pathValues === undefined || pathValues.length === 0) {
        const result = v.safeParse(route.paramsSchema, searchParams);
        invariant(result.success, "잘못된 파라미터입니다!\n" + routeKey + "\n\n" + JSON.stringify(searchParams))
        return result.output as RouteParams<RouteKey>;
    }
    
    const pathParams = Object.fromEntries(pathKeys.map((path, index) => {
